...
public class ColumnsController(IColumnService columnService) : ControllerBase
{
    ...
    public async Task<IActionResult> Delete(string tableName, string columnName, CancellationToken cancellationToken)
    {
        await _columnService.DeleteAsync(tableName, columnName, cancellationToken);
        return NoContent();
    }

    [HttpGet("{columnName}")]
    public async Task<IActionResult> GetByName(string tableName, string columnName, CancellationToken cancellationToken)
    {
        var column = await _columnService.GetAsync(tableName, columnName, cancellationToken);
