# File Management

This directory contains the split Simple Class Diagrams for the File Management component under the Storage Engine, organized by architectural concerns:

1. [Service Architecture Diagram](file:///c:/Users/ADMIN/Desktop/DBMS/docs/diagrams/class-diagrams/storage-engine/file-management/service-architecture.md)
   - Defines the service interfaces, their concrete implementations, and their structural associations.
2. [File Domain Model Diagram](file:///c:/Users/ADMIN/Desktop/DBMS/docs/diagrams/class-diagrams/storage-engine/file-management/domain-model.md)
   - Defines the static layout and structural compositions of the database files, headers, and allocations.
3. [Runtime File Management Diagram](file:///c:/Users/ADMIN/Desktop/DBMS/docs/diagrams/class-diagrams/storage-engine/file-management/runtime-management.md)
   - Explains the tracking of open files and how file entries own OS handles.
4. [Extent Management Diagram](file:///c:/Users/ADMIN/Desktop/DBMS/docs/diagrams/class-diagrams/storage-engine/file-management/extent-management.md)
   - Defines the interface and implementation class for extent-based space allocation.
